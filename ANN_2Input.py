...
test = merge.loc['2012-12-31':'2013-12-31'].values
train = merge.loc['1990-01-01':'2012-12-31'].values
...
# Generating flattened X_scaled for matlab input
flattened_X_scaled = []
for i in range(0, len(X_scaled)):
    vector = []
    for j in X_scaled[i, :, :]:
        vector = np.r_[vector, j]
    flattened_X_scaled.append(vector)
flattened_X_scaled = np.array(flattened_X_scaled)

#np.savetxt('flattened_X_scaled for ANN.csv',flattened_X_scaled,fmt='%f',delimiter=',')
#np.savetxt('Fowrate target for ANN.csv',flowrate.iloc[:, 0],fmt='%f',delimiter=',')

X_train = np.array(pd.DataFrame(flattened_X_scaled).dropna())
X_train = X_train[:338, :]
# Constructing a LSTM
...
opt = tf.keras.optimizers.SGD(learning_rate=0.001)#默认值0.001，先用默认值确定其他超参，learningRate和epoch一起在下面CV_Training确定
...
def create_ANN(neurons, dropoutRate, actFunc):
    ...
    regressor.compile(loss='mean_squared_error', optimizer=opt, metrics=['mse'])#adam may be changed later
    return regressor
...
r = regressor.fit(X_train.astype('float64'), y_train.astype('float64'), epochs=epochs_max, batch_size=batch_size, 
              callbacks=[early_stop_callback], validation_split=0.15)#转换成在validation set 上面验证
regressor.summary()
...
regressor = create_ANN(neurons=best_neurons,
                        dropoutRate=best_dropoutRate,
                        actFunc=best_actFunc)
r = regressor.fit(X_train.astype('float64'), y_train.astype('float64'), epochs=early_epoch, batch_size=batch_size)
plt.plot(r.history['loss'], label='loss')
plt.legend()
plt.show()

y_pred_scaled = regressor.predict(X_train.astype('float64'))
