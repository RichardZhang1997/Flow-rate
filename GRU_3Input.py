...
np.savetxt(station+'_GRU_Test_Data_ctrl_group.csv',np.c_[test_datetime,y_test_not_scaled,y_pred],fmt='%s',delimiter=',')

# Saving prediction on train set
np.savetxt(station+'_GRU_Train_Data_ctrl_group.csv',np.c_[train_datetime,y_train_not_scaled,y_pred_train],fmt='%s',delimiter=',')

# Saving the GRU weights
regressor.save_weights('./Vanilla_GRU results/'+station+'_3Input')
#regressor.save_weights('./GRU results/'+station+'_3Input')#Skip compiling and fitting process

# Restore the weights
#regressor.load_weights('./Vanilla_GRU results/'+station+'_3Input')#Skip compiling and fitting process
#regressor.load_weights('./GRU results/'+station+'_3Input')#Skip compiling and fitting process
